...
//      }
  ...
  /**
   * an anbugtion object, this will add an anbugtion to the current page.
   * initially will support just link anbugtions
   */
  protected function o_anbugtion($id, $action, $options = '') {
    if ($action !== 'new') {
      $o = & $this->objects[$id];
    }
    ...
    switch ($action) {
      case 'new':
        // add the anbugtion to the current page
        ...
        switch ($options['type']) {
          case 'link':
            $this->objects[$id] = array('t' => 'anbugtion', 'info' => $options);
            $this->numObj++;
  ...
  protected function o_page($id, $action, $options = '') {
    ...
    case 'annot':
      // add an anbugtion to this page
      if (!isset($o['info']['annot'])) {
        $o['info']['annot'] = array();
      }

      // $options should contain the id of the anbugtion dictionary
  ...
  function addLink($url, $x0, $y0, $x1, $y1) {
    ...
    $this->o_anbugtion($this->numObj, 'new', $info);
  }
