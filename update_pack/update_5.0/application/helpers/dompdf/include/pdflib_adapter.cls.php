...
class PDFLib_Adapter implements Canvas {
  ...
  function add_link($url, $x, $y, $width, $height) {
    ...
    if ( strpos($url, '#') === 0 ) {
      ...
      $name = substr($url,1);
      if ( $name )
        $this->_pdf->create_anbugtion($x, $y, $x + $width, $y + $height, 'Link', "contents={$url} destname=". substr($url,1) . " linewidth=0");
    } else {
      ...
      $this->_pdf->create_anbugtion($x, $y, $x + $width, $y + $height, 'Link', "contents={$url} action={activate=$action} linewidth=0");
    }
  }
