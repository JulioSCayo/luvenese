...
<html lang="en">
	...
	<?php
	    $paypal_activity = json_decode($this->crud_model->get_settings('paypal'));
	    $stripe_activity = json_decode($this->crud_model->get_settings('stripe_keys'));
	    $milestone_details = $this->db->get_where('project_milestone', array('project_milestone_id' => $project_milestone_id, 'status' => 0))->row_array();
			...
			<div class="col-md-8">
				<div class="row">
					<div class="col-md-12">
						...
						<!-- <a href="<?php // echo site_url('client/projectroom/payment/'.$project_code); ?>" class="close-btn-light float-right"><i class="fa fa-times"></i></a> -->
						<a href="javascript:;" onclick="window.close();" class="close-btn-light float-right"><i class="fa fa-times"></i></a>
					</div>
				</div>
			</div>
		</div>
			...
			<div class="col-md-8">
				<div class="row">
					<div class="col-md-3">
						...
						<?php if ($paypal_activity[0]->active == 1): ?>
						...
						<?php endif; if ($stripe_activity[0]->active == 1): ?>
					...
					<div class="col-md-8">
						<div class="w-100">
							<p class="pb-2 payment-header"><?php echo get_phrase('payment_milestone'); ?></p>
	                        <p class="item float-left">
								<span class="count-item">1</span>
								<span class="item-title mt-3"><?= $milestone_details['title']; ?>
									<span class="item-price">
										<?= $currency.' '.$milestone_details['amount']; ?>
									</span>
								<!-- </span>
								<span class="by-owner">
									owner name
						...
						<div class="w-100 float-left mt-4 indicated-price">
							<div class="float-right total-price"><?= $currency.' '.$milestone_details['amount']; ?></div>
							<div class="float-right total">
								<?= get_phrase('total'); ?>
							</div>
						</div>
						<div class="w-100 float-left">
							<form action="<?php echo site_url('client/paypal_checkout/'.$project_milestone_id); ?>" method="post" class="paypal-form form">
								<hr class="border mb-4">
								<button type="submit" class="payment-button float-right"><?php echo get_phrase('pay_by_paypal'); ?></button>
				            </form>
				            <form action="<?php echo site_url('client/stripe_checkout/'.$project_milestone_id); ?>" method="post" class="stripe-form form">
				            	<hr class="border mb-4">
