...
?>
	...
	<div class="tab-pane <?php 
        ...
        <?php echo form_open(site_url('admin/note/save/' . $row['note_id']), array(
        ...
        <button type="button" class="btn btn-white pull-right" id="submit-button" 
            onclick="confirm_modal('<?php echo site_url('admin/note/delete/' . $row['note_id']); ?>', 
                                    '<?php echo site_url('admin/reload_notes_tab_body'); ?>');">
...
<script>
    ...
    var post_refresh_url    =   '<?php echo site_url('admin/reload_notes_tab_body');?>';
</script>
...
<?php foreach ($notes as $row):?>
    ...
    function showResponse<?php echo $row['note_id'];?>(responseText, statusText, xhr, $form)  { 
        ...
        $.ajax({
        url: "<?php echo site_url('admin/reload_notes_tab_body/' . $row['note_id']);?>",
