    ...
    <div class="row">
            ...
            <div class="panel panel-primary">
                ...
                <div class="panel-body" style="padding:0px;">
                                    ...
                                    <article class="story" style="padding:0px 10px 0px 20px; margin:20px 0px;">
                                        <aside class="user-thumb">
                                            <a href="#">
                                                ...
                                                ?> " 
                                                     alt="" class="img-circle" style="height:40px; width: 40px;">
                                            </a>
                                        </aside>
                    ...
                    <?php if ($row['status'] == 'opened'): ?>
                        <?php
                        echo form_open(site_url('client/support_ticket_post_reply/' . $row['ticket_code']), array('class' => 'form-horizontal form-groups validate ticket-message-add',
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-9">
                                <select name="canned_message" class="form-control selectboxit" id="support_canned_message_id" onchange="return apply_macro(this.value)">
                                    <option><?php echo get_phrase('apply_macro'); ?></option>
                                    <?php
                                    $canned_message = $this->db->get('support_canned_message')->result_array();
                                    foreach ($canned_message as $canned_message):
                                        ?>
                                        <option value="<?php echo $canned_message['support_canned_message_id'] ?>">
                                            <?php echo $canned_message['title']; ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
        ...
        <div class="col-md-4">
            ...
            <button type="button" class="btn btn-primary btn-icon icon-left" 
                    onclick="showAjaxModal('<?php echo site_url('modal/popup/support_ticket_assign_staff/' . $row['ticket_code']); ?>');">
            ...
            <button type="button" class="btn btn-info btn-icon icon-left" 
                    onclick="showAjaxModal('<?php echo site_url('modal/popup/support_ticket_update_status/' . $row['ticket_code']); ?>');">
...
<script type="text/javascript">
    ...
    function apply_macro(support_canned_message_id) {

        $.ajax({
            url: '<?php echo site_url('admin/get_canned_message/'); ?>' + support_canned_message_id,
...
<script>
    ...
    {
        $.ajax({
            url: '<?php echo site_url('admin/reload_support_ticket_view_body/' . $row['ticket_code']); ?>',
                        success: function (response)
                        {
