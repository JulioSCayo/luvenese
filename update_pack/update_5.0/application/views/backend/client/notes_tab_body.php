...
?>
	...
	<div class="tab-pane <?php 
        ...
        <?php echo form_open(site_url('client/note/save/' . $row['note_id']), array(
        ...
        <!--<div  style="padding:5px; background-color: #FFFCEE; border: 1px #f3f3f3 solid;">-->
    		...
    		<div class="form-group">
    	        <div class="col-md-12" style="padding:0px; background-color: #FFFCEE; margin-bottom: 15px;">
        ...
        <button type="button" class="btn btn-white pull-right" id="submit-button" 
            onclick="confirm_modal('<?php echo site_url('client/note/delete/' . $row['note_id']); ?>', 
                                    '<?php echo site_url('client/reload_notes_tab_body'); ?>');">
...
<script>
    ...
    var post_refresh_url    =   '<?php echo site_url('client/reload_notes_tab_body');?>';
</script>
...
<?php foreach ($notes as $row):?>
    ...
    function showResponse<?php echo $row['note_id'];?>(responseText, statusText, xhr, $form)  { 
        ...
        $.ajax({
        url: "<?php echo site_url('client/reload_notes_tab_body/' . $row['note_id']);?>",
