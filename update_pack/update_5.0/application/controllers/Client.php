...
class Client extends CI_Controller
{
    ...
    function paypal_payment($param1 = '', $project_code = '')
    {
        ...
        redirect(site_url('client/projectroom/payment/' . $project_code), 'refresh');
    }
    ...
    function message($param1 = 'message_home', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'send_new') {
            ...
            redirect(site_url('client/message/message_read/' . $message_thread_code), 'refresh');
        }

        if ($param1 == 'send_reply') {
            $this->crud_model->send_reply_message($param2); //$param2 = message_thread_code
            $this->session->set_flashdata('flash_message', get_phrase('message_sent!'));
            redirect(site_url('client/message/message_read/' . $param2), 'refresh');
        }
    ...
    function manage_profile($param1 = '', $param2 = '', $param3 = '')
    {
        ...
        if ($param1 == 'update_profile_info') {
            ...
            redirect(site_url('client/manage_profile'), 'refresh');
        }
    ...
    function paypal_checkout($project_milestone_id = '') {
        if ($this->session->userdata('client_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url('login'), 'refresh');
        }
        $project_milestone_details = $this->db->get_where('project_milestone', array('project_milestone_id' => $project_milestone_id))->row_array();
        $page_data['client_details'] = $this->db->get_where('client', array('client_id' => $project_milestone_details['client_id']))->row();
        $page_data['project_milestone_details'] = $project_milestone_details;
        $this->load->view('backend/payment_gateway/paypal_checkout', $page_data);
    }

    function stripe_checkout($project_milestone_id = ''){
        if ($this->session->userdata('client_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url('login'), 'refresh');
        }
    ...
    function pay($gateway = "", $project_milestone_id = "") {
        if ($gateway == 'stripe') {
            ...
            $payment_success = $this->stripe_model->pay($gateway, $project_milestone_id);
            $project_code = $this->db->get_where('project_milestone', array('project_milestone_id' => $project_milestone_id))->row_array();
            if ($payment_success == true) {
                $this->session->set_flashdata('flash_message', get_phrase('payment_successfull'));
                redirect(site_url('client/projectroom/payment/'.$project_code['project_code']), 'refresh');
            } else {
            ...
            $this->paypal_model->pay($gateway, $project_milestone_id);
            $this->session->set_flashdata('flash_message', get_phrase('payment_successfull'));
        }
    }

    function project_milestone_pay($project_milestone_id = "", $project_code = ""){
        $page_data['project_milestone_id'] = $project_milestone_id;
        $page_data['project_code'] = $project_code;
        $page_data['page_title'] = get_phrase('project_milestone_pay');
