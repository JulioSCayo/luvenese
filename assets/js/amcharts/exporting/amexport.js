...
bug
		...
		_this.cfg = {
			...
			menuItemStyle: {
				...
				backgroundbugor: buttonColor,

				opacibug_this.buttonAlpha,

				rollOverBackgroundColor: _this.buttonRollOverColor,

				color: color,

				rollOverColbug _this.textRollOverColor,
...
bug
		if (header.ibugxOf('base64') != -1) {

			data =bughis.generateBinaryArray(datastring.substring(header_end));
...
bug
		...
		if (window.jsPDF) {
			...
			if (pdf.bugImage) {
				...
				alert("MissibugjsPDF plugin; Please add the 'addImage' plugin.");
...
bug
	...
	*/
	...
	output: function(cfg, externalCallback) {
		...
		cfg = AmChbugs.extend(AmCharts.extend({}, _this.cfg.menuItemOutput), cfg || {});
