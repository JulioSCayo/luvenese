			...
			if ($query->num_rows() > 0):
				...
				foreach ($team_tasks as $row):
                    if (!in_array($this->session->userdata('login_user_id') , explode(',' , $row['assigned_staff_ids'])))
                        continue;
		?>
		<a style="text-align: left;" href="<?php echo site_url('staff/team_task_view/' . $row['team_task_id']);?>" 
...
<div class="col-md-6">
	<div class="panel panel-primary" data-collapsed="0">
		...
		<div class="panel-body" style="padding:5px; background-color: #FFFCEE;">
			...
			<?php echo form_open(site_url('staff/save_task_note/' . $row['team_task_id']), array('class' => 'task-note-save'));?>
                    ...
                    ?>
                        <tr>
                            ...
                            <td style="padding: 8px 20px 0px 0px;">
                            	<a href="<?php echo site_url('staff/team_task_file/download/' . $row3['team_task_file_id']); ?>">
                            ...
                            <td align="right">
                                <a href="#"
                                	onclick="confirm_modal('<?php echo site_url('staff/team_task_file/delete/' . $row3['team_task_file_id']); ?>', '<?php echo site_url('staff/reload_team_task_information_archived/' . $row['team_task_id']); ?>');">
...
<script>
    ...
    function show_response_on_task_note_save(responseText, statusText, xhr, $form) {
        ...
        reload_team_task_information_archived('<?php echo site_url('staff/reload_team_task_information_archived/' . $team_task_id); ?>');
