...
<center>
    ...
    <?php echo form_open(site_url('admin/email_milestone_invoice/' . $row['project_milestone_id']), array('class' => ' invoice-send pull-right', 'style' => 'margin:0px 5px;' ));?>
                
         <span id="preloader-form"></span>
        <input type="hidden" name="invoice_body" id="invoice_to_send" value="">
        <button type="submit" class="btn btn-default btn-icon icon-left" onclick="if(!confirm('<?php echo get_phrase("A_email_will_be_sent_to"); ?>'+' <?php echo $this->db->get_where('client', array('client_id' => $row['client_id']))->row('email'); ?> '+'<?php echo get_phrase('with_invoice'); ?>')) return false;" id="submit-button">
    ...
    <div id="invoice_print">
        <table width="100%" border="0">
            <tr>
                <td width="50%"><img src="<?php echo base_url('assets/dark-logo.png');?>" style="max-height:80px;"></td>
                <td align="right">
